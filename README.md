...
- **常時シークレットモード** - Cookie、履歴、キャッシュを一切保存しない（デフォルト有効）
...
- 常時シークレットモード（Cookie・履歴・キャッシュ保存なし）
...
### シークレットモード
- **常時有効**: Cookie、履歴、キャッシュは一切保存されません
- ブラウザを閉じると、すべてのデータが自動的に削除されます
- Chromeのシークレットモードと同等のプライバシー保護
